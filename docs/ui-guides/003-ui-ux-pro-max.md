---
title: "UI UX Pro Max"
category: "UI/UX Design Intelligence"
format: "SKILL.md"
stack: "React, Next.js, Vue, Svelte, Tailwind, shadcn/ui, HTML/CSS"
source: "https://github.com/nextlevelbuilder/ui-ux-pro-max-skill"
install: "npx skills add nextlevelbuilder/ui-ux-pro-max-skill"
---

# UI UX Pro Max

## What this is
A frontend/UI-focused Markdown-style agent instruction source or rule reference. This entry is a curated pointer and usage card, not a copied mirror of the original file.

## Best for
Use for dashboards, SaaS pages, landing pages, cards, forms, typography, palettes, and UX review.

## Stack / keywords
React, Next.js, Vue, Svelte, Tailwind, shadcn/ui, HTML/CSS

## Source
https://github.com/nextlevelbuilder/ui-ux-pro-max-skill

## Install or use
```bash
npx skills add nextlevelbuilder/ui-ux-pro-max-skill
```

## Prompt to give your coding agent
```text
Use `UI UX Pro Max` as the frontend/UI guidance for this task. Follow its conventions for component structure, styling, accessibility, responsive behavior, and design consistency. Prefer reusable components, semantic markup, clear loading/error/empty states, and production-ready UI polish.
```

## Notes
- Review the source before installing third-party agent skills or rules.
- Keep this file as a shortcut index in your project docs, then install or copy the actual rule/skill from the linked source when needed.
